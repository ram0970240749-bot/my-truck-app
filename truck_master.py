...
if 'trucks_db' not in st.session_state:
    st.session_state['trucks_db'] = pd.DataFrame([
        {"ทะเบียนรถ": "TRK-01", "ชื่อคนขับ": "สมชาย ใจดี", "สิ่งที่บรรทุก": "ข้าวสาร 20 ตัน", "เวลาคาดว่าจะถึง (ETA)": "14:30", "สถานะ": "กำลังวิ่ง", "จังหวัดต้นทาง": "ปทุมธานี", "จังหวัดปลายทาง": "กรุงเทพมหานคร", "ค่าน้ำมัน (บาท)": 2500, "กำไรต่อเที่ยว (บาท)": 4500, "lat": 13.7563, "lon": 100.5018},
        {"ทะเบียนรถ": "TRK-02", "ชื่อคนขับ": "วิชัย รักดี", "สิ่งที่บรรทุก": "เหล็กเส้น 15 ตัน", "เวลาคาดว่าจะถึง (ETA)": "18:00", "สถานะ": "จอดพัก", "จังหวัดต้นทาง": "สระบุรี", "จังหวัดปลายทาง": "นครราชสีมา", "ค่าน้ำมัน (บาท)": 3000, "กำไรต่อเที่ยว (บาท)": 6200, "lat": 14.9738, "lon": 102.0836},
        {"ทะเบียนรถ": "TRK-03", "ชื่อคนขับ": "อนันต์ ยอดเยี่ยม", "สิ่งที่บรรทุก": "อาหารแช่แข็ง", "เวลาคาดว่าจะถึง (ETA)": "ถึงแล้ว", "สถานะ": "รถพัง/ชำรุด", "จังหวัดต้นทาง": "ระยอง", "จังหวัดปลายทาง": "ชลบุรี", "ค่าน้ำมัน (บาท)": 2200, "กำไรต่อเที่ยว (บาท)": 3800, "lat": 12.9236, "lon": 100.8824}
    ])

# ==========================================
# 🔐 ระบบจัดการล็อกอิน (Login System) 3 ฝ่าย
# ==========================================
...
if not st.session_state['logged_in']:
    ...
    def render_thai_map_with_colors(data_df):
        st.subheader("🗺️ แผนที่พิกัดเรียลไทม์แสดงสถานะรถ")
        ...
        # ตั้งค่ามุมมองเริ่มต้นให้อยู่กึ่งกลางประเทศไทย
    ...
    # ==========================================
    # 👑 สิทธิ์ที่ 1: เจ้าของธุรกิจ (แก้ไขหรือกรอกได้ทุกช่องทุกแถว)
    # ==========================================
    if st.session_state['user_role'] == "เจ้าของธุรกิจ (Owner)":
        ...
        with tab1:
            st.write("💡 เถ้าแก่สามารถดับเบิ้ลคลิกแก้ไขข้อมูล ตัวเลขเงิน หรือข้อมูลจังหวัดในตารางนี้ได้ครบทุกคอลัมน์:")
    ...
    # ==========================================
    # 💵 สิทธิ์ที่ 2: พนักงานบัญชี (เห็นทั้งหมด แต่ล็อกตารางไม่ให้แก้ไข)
    # ==========================================
        ...
        with tab1:
            st.info("🔒 สิทธิ์พนักงานบัญชี: สามารถดูข้อมูลค่าน้ำมัน จังหวัดส่งของ และกำไรต่อเที่ยวได้ทั้งหมด แต่ระบบปิดการแก้ไขช่องข้อมูล")
    ...
    # ==========================================
    # 🚛 สิทธิ์ที่ 3: พนักงานขับรถ (ซ่อนคอลัมน์เงินกำไรเด็ดขาด + มีฟอร์มกรอกงาน)
    # ==========================================
        ...
        with tab1:
            st.info("พี่ๆ คนขับกรอกรายละเอียดงานปัจจุบันและจังหวัดลงในช่องด้านล่าง แล้วกดส่งข้อมูลอัปเดต")
            ...
            with st.form("driver_input_form"):
                ...
                input_src = st.text_input("📍 จังหวัดต้นทาง (จังหวัดที่รับของมา):")
                input_dest = st.text_input("🏁 จังหวัดปลายทาง (จังหวัดที่จะไปส่งสินค้า):")
                ...
                if submit:
                    if input_truck_id == "" or input_driver_name == "":
                        ...
                        # พิกัดจำลองกระจายตัวตามจังหวัดหลัก
                        prov_coords = {"กรุงเทพมหานคร": [13.7563, 100.5018], "นครราชสีมา": [14.9738, 102.0836], "ชลบุรี": [12.9236, 100.8824]}
                        coords = prov_coords.get(input_dest, [13.7563, 100.5018])
                        
                        if input_truck_id in df['ทะเบียนรถ'].values:
                            st.session_state['trucks_db'].loc[df['ทะเบียนรถ'] == input_truck_id, ['ชื่อคนขับ', 'สิ่งที่บรรทุก', 'เวลาคาดว่าจะถึง (ETA)', 'สถานะ', 'จังหวัดต้นทาง', 'จังหวัดปลายทาง', 'lat', 'lon']] = [input_driver_name, input_cargo, input_eta, input_status, input_src, input_dest, coords[0], coords[1]]
                        else:
                            new_row = pd.DataFrame([{
                                "ทะเบียนรถ": input_truck_id, "ชื่อคนขับ": input_driver_name, 
                                "สิ่งที่บรรทุก": input_cargo, "เวลาคาดว่าจะถึง (ETA)": input_eta, 
                                "สถานะ": input_status, "จังหวัดต้นทาง": input_src, "จังหวัดปลายทาง": input_dest,
                                "ค่าน้ำมัน (บาท)": 0, "กำไรต่อเที่ยว (บาท)": 0,
