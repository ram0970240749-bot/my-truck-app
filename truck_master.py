...
if not st.session_state['logged_in']:
    ...
    def render_thai_map(data_df):
        ...
        # ใช้แผนที่ฐานแบบสว่างที่เน้นภาษาท้องถิ่น (ภาษาไทย)
        view_state = pdk.ViewState(latitude=13.7563, longitude=100.5018, zoom=5, pitch=0)
        layer = pdk.Layer(
            "ScatterplotLayer",
            data_df,
            get_position="[lon, lat]",
            get_color="[255, 0, 0, 200]" if st.session_state['user_role'] != "พนักงานขับรถ (Driver)" else "[0, 0, 255, 200]",
            get_radius=20000,
            pickable=True,
        )
        r = pdk.Deck(
            ...
            map_style="mapbox://styles/mapbox/streets-v11", # รูปแบบแผนที่มาตรฐานที่แสดงชื่อเมือง/จังหวัดภาษาไทยตามเครื่องผู้ใช้
    ...
    if st.session_state['user_role'] == "เจ้าของธุรกิจ (Owner)":
        ...
        with tab1:
            ...
            # เปิดตารางให้แก้ไขได้สดๆ และเพิ่มแถวได้เต็มรูปแบบ
    ...
    # ==========================================
        ...
        with tab1:
            ...
            # ใช้ st.dataframe แทน st.data_editor เพื่อให้ดูได้อย่างเดียว ล็อกไม่ให้แก้ช่องข้อมูล
    ...
    # ==========================================
            ...
            with st.form("driver_input_form"):
                ...
                if submit:
                    if input_truck_id == "" or input_driver_name == "":
                        ...
                        # พิกัดคร่าวๆ ในไทยสำหรับการแสดงผลบนแผนที่
                        new_lat = 13.7563
                        new_lon = 100.5018
        ...
        with tab2:
            # สร้างข้อมูลจำลองฝั่งคนขับโดยการลบคอลัมน์ "กำไรต่อเที่ยว (บาท)" ออกอย่างเด็ดขาดตามเงื่อนไข
